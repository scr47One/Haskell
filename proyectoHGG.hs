...
-- [x:xs] x es la cabeza de la lista, xs
diferentesC :: [(Integer,String)] -> [String]
...
sumamos el valor asociado del numero romano y comparamos al siguiente, en otro caso nos vamos al siguiente valor en la lista a comparar
 -}
sumaDif :: [String]-> [(Integer, String)]->Integer
sumaDif _ []= 0
sumaDif [] _ = 0
sumaDif (v:xs)  ((n,m):bv) = if v == m then n+sumaDif xs ((n,m):bv)
else sumaDif ((v:xs)) bv
